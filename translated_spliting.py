...
trans_df=pd.read_excel('F:/hindi_english_downloaded_split/epub/Hindi_all_sentences_trans.xlsx')
for io in range(len(list_dir0)):
    ...
    for j in range(len(list_dir)):
        ...
        translated_1.extend(trans_df['Translated'][pos_sec:pos_sec+len(df_section)])
        pos_sec=pos_sec+len(df_section)
