...
def Hindi_Book(Book_Name):
    ...
    Hindi_Final_List=[]
    for i in range(len(Hindi_Txt_List)):
        s1=convertToUni(str(Hindi_Txt_List[i]), 1)
        Hindi_Final_List.append(s1)
    s2=' '.join(map(str, Hindi_Final_List))
    File1_Open.close()
    file1 = open(str(Book_Name)[:-4]+".txt","w",encoding="utf8")
    file1.write(s2)
    file1.close()
os.chdir("F:/hindi_english_downloaded_split/change")
fle=os.listdir()
for i in fle:
    Hindi_Book(i)
