# -*- coding: utf-8 -*-
"""
Created on Thu Nov 28 15:46:53 2019
...
def PreProcess_Science(Hindi_String_list,English_String_list):
    ...
    for i in range(1,len(l1)):
        try:
            ...
            try:
                l=re.findall("^[ ]*[\u0900-\u097F]*[ ]*"+chapter_n+"[\.\-][\d\.]+",l1[i])
                l4.append(l[0])
            except:
                l4.append(l)
    for i in range(1,len(l2)):
        try:
            ...
            try:
                l=re.findall("^[ ]*[A-Za-z]*[ ]*"+chapter_n+"[\.\-][\d\.]+",l2[i])
                l5.append(l[0])
            except:
                l5.append(l)
    #print(l4)
...
def training_data_section():
    ...
    global section_hindi
    global section_english
    print(len(section_hindi),len(section_english))
...
def training_data_paragraph():
    ...
    df["English"]=[]
    try:
        df=pd.read_csv("E:/books1/science/train_paragraph_level.csv")
    except:
        print("file created")
    try:
        df=df.drop(["Unnamed: 0"],axis=1)
...
def training_data_sentence():
    ...
    df["English"]=[]
    try:
        df=pd.read_csv("E:/books1/science/train_sentence_level.csv")
    except:
        print("file created")
    try:
        df=df.drop(["Unnamed: 0"],axis=1)
