...
import re
df=pd.read_excel('F:/hindi_english_downloaded_split/epub/Hindi_all_sentences.xlsx')
for i in range(len(df)):
    s=str(df["Hindi"][i])
    if len(re.findall("[\u0900-\u097F]",s))<=len(re.findall("[A-Za-z]",s)):
        s1=convertToUni(s, 1)
        df["Hindi"][i]=s1

import os
os.chdir("F:/hindi_english_downloaded_split/epub")
df.to_excel("Hindi_all_sentences_V1.xlsx",index=False)
