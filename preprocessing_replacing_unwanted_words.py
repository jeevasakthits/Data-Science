# -*- coding: utf-8 -*-
"""
Created on Fri Nov 29 17:13:29 2019
...
def remove_unwanted_english(list1):
    for i in range(1,len(list1)):
        l=re.findall("([A-Za-z]+(.*?)[A-Za-z]+\.)",list1[i])
        k=[]
...
def training_data_paragraph():
    ...
    df["English"]=[]
    try:
        df=pd.read_csv("E:/hindi_and_english_books/train_paragraph_level.csv")
    except:
        print("file created")
    try:
        df=df.drop(["Unnamed: 0"],axis=1)
...
def training_data_sentence():
    ...
    df["Translated"]=[]
    try:
        df=pd.read_csv("E:/hindi_and_english_books/train_sentence_level.csv")
    except:
        print("file created")
    try:
        df=df.drop(["Unnamed: 0"],axis=1)
